<?php 
	...
	else if($get_method['method_name']=="get_home_banner")
	{
		...
		while($data = mysqli_fetch_assoc($sql_all))
		{
			...
			$row2['place_name'] = ($slider_title!='') ? $slider_title : '';
	...
	{	 
		...
		if($user_result->num_rows > 0)
		{
			...
			$sql="UPDATE tbl_users SET `password`='$new_password' WHERE `id`='".$user_row['id']."'";
	...
	// Get favourite place list start
	...
	// Get favourite place list end

	// Get ratings place start
	...
	// Get ratings place end

	// Get user ratings start
	else if($get_method['method_name']=="get_user_rating")
	{
		...
		if(mysqli_num_rows($res) > 0){
			...
			$set['PLACE_APP'][]=array('user_rate' => $user_rate['rate'],'user_msg' => $user_rate['message'],'msg' => $app_lang['user_rate'],'success'=>"1");

		}else{

			$set['PLACE_APP'][]=array('user_rate' => "0",'user_msg' => "",'msg' =>$app_language['user_not_rate'],'success'=>"1");
		}
	...
	// Get app details start	
		...
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$row['native_cat_position'] = $data['native_cat_position'];
