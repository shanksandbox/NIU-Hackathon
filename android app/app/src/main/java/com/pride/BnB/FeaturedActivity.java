...
public class FeaturedActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        JsonUtils.setStatusBarGradiant(FeaturedActivity.this);
        toolbar.setTitle(R.string.home_slider);
        ...
        recyclerView.setLayoutManager(new GridLayoutManager(FeaturedActivity.this, 1));
        recyclerView.setFocusable(false);
        adLayout = findViewById(R.id.adLayout);
        if (Constant.SAVE_BANNER_TYPE.equals("admob")) {
            JsonUtils.ShowBannerAds(FeaturedActivity.this, adLayout);
        } else {
            JsonUtils.ShowBannerAdsFb(adLayout, FeaturedActivity.this);
        }
        no_fav = findViewById(R.id.no_fav);


        JsonObject jsObj = (JsonObject) new Gson().toJsonTree(new API());
        jsObj.addProperty("method_name", "featured_place");
        jsObj.addProperty("user_id", MyApplication.getAppInstance().getUserId());
        if (JsonUtils.isNetworkAvailable(FeaturedActivity.this)) {
            new getPopularData(API.toBase64(jsObj.toString())).execute(Constant.API_URL);
        }

    }

    @SuppressLint("StaticFieldLeak")
    private class getPopularData extends AsyncTask<String, Void, String> {

        String base64;

        private getPopularData(String base64) {
            this.base64 = base64;
        }
    ...
    private void displayData() {
        adapter = new SearchAdapter(FeaturedActivity.this, mListItem);
