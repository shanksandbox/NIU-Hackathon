...
public class PopularActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        JsonUtils.setStatusBarGradiant(PopularActivity.this);
        toolbar.setTitle(R.string.home_popular);
        ...
        recyclerView.setLayoutManager(new GridLayoutManager(PopularActivity.this, 1));
        recyclerView.setFocusable(false);
        adLayout = findViewById(R.id.adLayout);
         if (Constant.SAVE_BANNER_TYPE.equals("admob")) {
            JsonUtils.ShowBannerAds(PopularActivity.this, adLayout);
        } else {
            JsonUtils.ShowBannerAdsFb(adLayout, PopularActivity.this);
        }
        no_fav=findViewById(R.id.no_fav);


            JsonObject jsObj = (JsonObject) new Gson().toJsonTree(new API());
            jsObj.addProperty("method_name", "popular_place");
            jsObj.addProperty("user_id", MyApplication.getAppInstance().getUserId());
            if (JsonUtils.isNetworkAvailable(PopularActivity.this)) {
    ...
    private void displayData() {
        adapter = new SearchAdapter(PopularActivity.this, mListItem);
